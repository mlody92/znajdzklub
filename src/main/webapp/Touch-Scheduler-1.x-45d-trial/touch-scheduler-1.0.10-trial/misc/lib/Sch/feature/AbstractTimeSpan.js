...
Ext.define("Sch.feature.AbstractTimeSpan", {
    ...
    init:function(scheduler) {
        ...
        if (Ext.versions.touch) {
            eventSource = this.panel;
        } else{
            eventSource = this.panel.normalGrid;
        }
    ...
    onViewReady : function () {
        var view            = this.schedulerView = this.panel.getSchedulingView();

        this.containerEl    = view.getSecondaryCanvasEl();
        this.timeAxis       = this.panel.getTimeAxis();
...
//                });
        ...
        if (rowContainer && rowContainer.down('.sch-timetd', true)) {
            this.renderElements();
        }
    },
