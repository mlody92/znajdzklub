...
//    barMargin           : null,
    ...
    */
     ...
     */
    getElementFromEventRecord: function (record, all) {
        if (all === true) {
            return this.getEventNodesByRecord(record);
        } else {
            if (this.getMode() === 'calendar') {
                return this.getEventNodesByRecord(record).item(0);
            } else {
    ...
    */
    getEventIdFromDomNodeId: function (id) {
        if (this.getMode() === 'calendar') {
            var internalIdArray = id.substring(this.eventPrefix.length).split('-');
            internalIdArray.pop();

            return internalIdArray.join('-');
        } else {
    ...
    * @param {String} mode Either 'horizontal', 'vertical' or 'caledar'
    */
